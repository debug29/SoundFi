...
-(SoundFiAudioSession*)initWithSpecificRateAndBufferSize:(SFSampleRateValue)rate : (SFBufferSizeValue)bufferSize{
    ...
    nbrEchantillon=bufferSize; //1024
    sampleRate = rate;
    //Notification listening for background and foreground
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(enteredBackground)
...
-(void)messagingReceptionSampleTreatment{
    ...
    if(isInitiate) {
        ...
        if (sampleFrequency>=userMinFreq-5 && sampleFrequency<=userMaxFreq) {                 //Ajout d'un caractère à la chaine finale
            //NSString *res = [NSString stringWithFormat:@"%c",(char)((sampleFrequency-17995)/18+32)];
            NSString *res;
            @try {
                res = [userDictionnary objectAtIndex:((sampleFrequency-userMinFreq-5)/userFreqEspacement)];
            }
        ...
        else if(sampleFrequency>=userStopFreq-9 && sampleFrequency<=userStopFreq+9) {             //Fin du message
#if DEBUG
            NSLog(@"Stop");
#endif
