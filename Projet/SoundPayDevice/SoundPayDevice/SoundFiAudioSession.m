                                        ...
                                        SInt16                      *sampleBuffer)      // frame buffer
{
	...
	if (read > inNumberFrames) {
        ...
        for (int i=100; i<n; i+=2) {
			...
			if ((curFreq > dominantFrequency) && (curFreq >10000000000)) {
...
-(void)sampleTreatment:(int)numFrames
{
    if(isInitiate)
    {
        ...
        if (sampleFrequency>=17995 && sampleFrequency<=19711)
        {
            NSString *res = [NSString stringWithFormat:@"%c",(char)((sampleFrequency-17991)/18+32)];
            [messageReceive appendString:res];
            compteur=0;
        }
        else if(sampleFrequency>=19720 && sampleFrequency<=19736)
        {
            NSLog(@"Stop");
            [self startAnalysis];
            isInitiate=FALSE;
    ...
    else
    {
        ...
        if (sampleFrequency>=17750 && sampleFrequency<17950 && !isInitiate)
        {
            NSLog(@"Gogo");
            receptionQuality=0;
...
-(void)checkProcessTimeOut
{
    if (compteurProcess>timeOutProcess) {
        NSLog(@"TimeOut");
        [self.delegate messageReceived:@"TimeOut" :-1001];
...
////////////
//
//  Check each word with the UISpellChecker
//  NOTE : don't check URL -> error
//
////////////
-(void) analysisPhase6
{
...
-(void)startAnalysis
{
    ...
    //NSLog(@"Pré 6 : %@",messageReceive);
    
    //NSLog(@"Analysis phase 6");
    //[self analysisPhase6];
    
    NSLog(@"RES /**/**/**/ : %@",messageReceive);
    //NSLog(@"Reception quality : %d",receptionQuality);
    
    [self.delegate messageReceived:[NSString stringWithString:messageReceive] :receptionQuality];
...
////////////
//
//  Controle the app volume for sending message
//  return 1 if volume > 0.9
//
///////////
-(int)volumeControl
{
...
////////////////
//
//  Control state of the transaction and send message
    ...
    switch (transactionState) {
        case 0: {                                                       // Acquitement du prix reçu
            ...
            dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self startAudioUnit:1:theMessage];
            });
        }
            break;
        case 1:                                                         // Si on reçoit "GP" le prix reçu est bon et on acquite GP sinon retour 0
            if (timeOut) {
                ...
                if ([theMessage isEqualToString:@"GP"]) {
