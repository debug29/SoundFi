...
-(void)initGeoloc{
    ...
    curentFrequencyData = malloc(sizeof(int)*50);
    for (int i=0; i<50; i++) {curentFrequencyData[i]=1;}
...
-(void)sampleTreatment:(int)numFrames {
    ...
    if(sampleFrequency>20100 && sampleFrequency <20300)
        NSLog(@"François 1 , GOOGLE 0");
...
-(void) zoneDetection:(int*)data
{
    ...
    for (int i=0; i<50; i++) {avrgFreq+=data[i];}
    avrgFreq/=50;

    //Calcul de l'écart type pour détecter les passages entre 2 zones
    for (int i=0; i<50; i++) {ecartType+=pow(data[i]-avrgFreq,2);}
    ecartType=sqrt(ecartType/50.);
    ...
    if ([lastPos isEqualToString:currentPos] && ![lastPosValidate isEqualToString:currentPos]) { //TEST
        ...
        [self.delegate localisationData:lieu :urlContents];
