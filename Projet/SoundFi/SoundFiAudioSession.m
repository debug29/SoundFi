...
OSStatus fftGetFrequencyHighAccuracy (
    ...
    osamp = 4;
    fftSize = 2048;		// this seems to work in real time since we are actually doing the fft on smaller windows
                                        ...
                                        SInt16                      *sampleBuffer)      // frame buffer
{
	...
	if (read > inNumberFrames) {
        ...
        for (int i=100; i<n; i+=2) {
            ...
            //printf("%d\n",(int)curFreq);
            if ((curFreq > dominantFrequency) && (curFreq >500000000)) {
...
-(void)sampleTreatment:(int)numFrames
{
    if(isInitiate) {
        ...
        if (sampleFrequency>=17995 && sampleFrequency<=19710) {
...
-(void)setupCallback
{
    ...
    emptySample=malloc(2048*sizeof(AudioUnitSampleType));
    for (int i=0; i<2048; i++) {
        emptySample[i]=0;
    }
    
    samplesBuffer=(SInt16*)malloc(2048 *sizeof(float));
}
...
-(void)fftSetup
{
    UInt32 maxFrames = 2048;    // fft size
...
-(int)initAudioSession
{
    ...
    [mySession setPreferredIOBufferDuration: (2048.0/sampleRate) error: &audioSessionError];
...
-(void)routeChanged
{
    NSLog(@"J'ai détecter un changement dans le périphérique d'entrée / sortie");
}
...
-(int)startAudioUnit:(int)mode : (NSString*)message
{
    ...
    else
    {
        ...
        if (!emissionMode)
        {
            ...
            NSData *temp = [message dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
            myMessage = [[NSString alloc] initWithData:temp encoding:NSASCIIStringEncoding];
