...
-(void)initUI
{
    ...
    //Bouton envoie message **Dialogue stuff**
    progSendBut = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    progSendBut.frame = CGRectMake(260,msgTextView.y + 129, 60, 30);
    progSendBut.layer.borderWidth=1.0f;
    progSendBut.layer.borderColor= [firstColor CGColor];
    [progSendBut setTitleColor:firstColor forState:UIControlStateNormal];
    [progSendBut setTitle:@"Log" forState:UIControlStateNormal];
    [progSendBut addTarget:self action:@selector(logSoundFi) forControlEvents:UIControlEventTouchUpInside];
    progSendBut.enabled=FALSE;
    progSendBut.alpha=0.;
    [self.view addSubview:progSendBut];
...
-(void)logSoundFi
{
    ...
    messageToSend = @"soundfi@gmail.com&123soleil";
...
{
    sendingProgress.progress=0;
    messageToSend = [NSString stringWithFormat:@"%@",[msgTextView text]];
