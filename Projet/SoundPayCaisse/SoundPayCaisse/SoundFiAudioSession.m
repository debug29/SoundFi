...
-(void)sampleTreatment:(int)numFrames
{
    ...
    else
    {
        ...
        if (sampleFrequency>=17650 && sampleFrequency<17950 && !isInitiate)
        {
            NSLog(@"Gogo");
            receptionQuality=0;
...
-(void)checkProcessTimeOut
{
    if (compteurProcess>timeOutProcess) {
        ...
        [NSString stringWithString:messageReceive];
    }
...
- (void) transactionUpdate:(NSString*) theMessage{
    ...
    switch (transactionState) {
        case 0:                                                                             // Envoie du prix et id transaction
            timeOutProcess=516;
            transactionState++;
            [self startAudioUnit:1:prix];
            break;
        case 1:                                                                             // Envoie GP si acquitement bon
            if (timeOut) {
                transactionState--;
                ...
                if ([theMessage isEqualToString:prix]) {
                    NSLog(@"Good price");
                    timeOutProcess=20000;                       // Remet le timeout à l'infini
                    transactionState++;
                    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [self startAudioUnit:1:@"GoodPrice"];
                    });
                }
                else {
                    NSLog(@"Nop");
            ...
            break;
        case 2: {                                                                          // Acquitement des ids client reçu
            idClient=theMessage;
            ...
            break;
        case 3:                                                                            // Si GID alors les ids sont bon je les envoies au serv
            if ([theMessage isEqualToString:@"GoodId"]) {
                //NSLog(@"%@",idClient);
                //NSLog(@"LAAAAAAAAA :::::: %@",[AESCrypt decrypt:idClient password:@"gjt:;=PLght54dcgxXDS"]);      // Test de décryptage
                NSLog(@"J'ai bien reçu les ID");
                [self.delegate clientData:idClient];
                dispatch_async(dispatch_get_main_queue(), ^{
                    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"SERVEUR"
                                                                     message:@"Taper sur un des boutons pour simuler une requete serveur"
                                                                    delegate:self
                                                           cancelButtonTitle:@"Succes" otherButtonTitles:@"Failure", nil];
                    [alert show];
                });
            }
            else {
            ...
            break;
        case 4: {                                                                           // Envoie du staut du paiement au client
            NSLog(@"Statut du paiement");
            [self.delegate transactionFinalState:statutPaiement];
            timeOutProcess=172;        //Time out de 1 sec
            dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self startAudioUnit:1:statutPaiement];
            });
            transactionState++;
        }
            break;
        case 5:                                                                             // Si acquit bon envoie GF, fin de la transaction
            if (timeOut) {
                transactionState--;
                [self transactionUpdate:nil];
            }
                ...
                if ([theMessage isEqualToString:statutPaiement]) {
                    NSLog(@"GG transaction terminé");
                    transactionState++;
                    timeOutProcess=20000;                                                               // Remet le timeout à l'infini
                    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [self startAudioUnit:1:@"GoodEnd"];
                    });
                    [self stopProcessingAudio];
                }
                else {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 0) {  // SUCCES
        statutPaiement = @"{valid:\"1\"}";
    }
    else {                   // FAILURE
        statutPaiement = @"{valid:\"0\"}";
    }
    transactionState++;
    [self transactionUpdate:nil];
}
