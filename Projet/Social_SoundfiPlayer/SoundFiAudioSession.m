...
@public
    ...
    //Frequency range
    int     startFrequencyRange;
    int     frequencySpacing;
    
    //Monofrequency emission
    int     monoFrequency;
    BOOL    sendMonoFrequency;
}
                                     ...
                                     SInt16                      *sampleBuffer)
{
    ...
    if (read > inNumberFrames) {
        ...
        for (int i=100; i<n; i+=2) {
            ...
            if ((curFreq > dominantFrequency) && (curFreq >50000)) { //1000000000 is the minimal "amplitude" to accept the pic
...
OSStatus renderToneCallback(void *userData,
    ...
    if (THIS->sendMonoFrequency) {
        int frequence = THIS->monoFrequency;
...
-(SoundFiAudioSession*)init
{
    ...
    mySession = [[AVAudioSession alloc]init];
    ...
    userStartFreq=17800;
    userStopFreq=19728;
    userFreqEspacement=18;
    numberOfAlphaInDictionnary=94;
    userDictionnary = [[NSMutableArray alloc]init];
    NSString *alpha = @" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
    for (int i =0; i<numberOfAlphaInDictionnary; i++) {
        [userDictionnary addObject:[NSString stringWithFormat:@"%c",[alpha characterAtIndex:i]]];
    }
...
-(SoundFiAudioSession*)initWithSpecificRateAndBufferSize:(SFSampleRateValue)rate : (SFBufferSizeValue)bufferSize{
    self=[super init];
    ...
    nbrEchantillon=bufferSize; //2048
    sampleRate = rate;
    //Notification listening for background and foreground
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(enteredBackground)
...
-(int)initAudioStreams
{
    ...
    //AVAudioSession *audioSession = mySession;
...
-(void)fftSetup {
    ...
    fftLog2n = log2f(maxFrames);            // log base2 of max number of frames, eg., 10 for 2048
    fftN = 1 << fftLog2n;					// actual max number of frames, eg., 2048 - what a silly way to compute it
...
-(int)initAudioSession
{
    ...
    [mySession setCategory: AVAudioSessionCategoryMultiRoute withOptions:AVAudioSessionCategoryOptionMixWithOthers error: &audioSessionError];
    ...
    // and so the fft size in the code 2048 -> 2048 everywhere
...
-(void)emissionSampleCalcul:(int)frequence : (int)numFrames : (Float32 *)buffer
{
    ...
    if (frequence == userStartFreq && amplitude < 0.8){
        amplitude+=0.04;
        //NSLog(@"Amplitude : %f",amplitude);
    }
    else if (frequence == userStopFreq && amplitude > 0)
        amplitude-=0.04;
    
    if (sendMonoFrequency)
        amplitude=0.7;
...
-(int)getASCIIFrequency
{
    ...
    if (initSequence) {
        ...
        nbrRepeatInit++;
        //NSLog(@"Freq init : %d",userStartFreq);
        return userStartFreq;
    }
    
    if (compteur < [myMessage length]) {
        frequence=userMinFreq +( [self getIndex:[NSString stringWithFormat:@"%c",[myMessage characterAtIndex:compteur]]]*userFreqEspacement);
        //NSLog(@"Lettre : %@  // At freq : %d",[NSString stringWithFormat:@"%c",[myMessage characterAtIndex:compteur]],frequence);
        if (frequence==-userMinFreq) {
            NSLog(@"Not in dico");
            frequence=0;
        }
    }
        ...
        //NSLog(@"Freq stop : %d",userStopFreq);
        return userStopFreq;
    }
...
-(void)sampleTreatment:(int)numFrames {
    ...
    */
    ...
    if (sampleFrequency>15000) {
        [self.delegate performSelector:@selector(findUserWithFreq:) withObject:[NSNumber numberWithInt:sampleFrequency]];
        //test ++;
        //NSLog(@"%d",test);
    }
}
...
-(void)messagingReceptionSampleTreatment{
    ...
    if(isInitiate) {
        ...
        if (sampleFrequency>=userMinFreq-(frequencySpacing/2) && sampleFrequency<=userMaxFreq+(frequencySpacing/2)) {
                                                                                                    //Ajout d'un caractère à la chaine finale
            //NSString *res = [NSString stringWithFormat:@"%c",(char)((sampleFrequency-17995)/18+32)];
            NSString *res;
            @try {
                res = [userDictionnary objectAtIndex:round(((float)(sampleFrequency-userMinFreq)/userFreqEspacement))];
                //NSLog(@"%d    %@",sampleFrequency,[userDictionnary objectAtIndex:round(((float)(sampleFrequency-userMinFreq)/userFreqEspacement))]);
            }
            @catch (NSException *exception) {
                NSLog(@"Exception:%@",exception);
                res=@"";
            }
            //NSLog(@"Frequency : %d // Res : %@",sampleFrequency,res);
            [messageReceive appendString:res];
            compteur=0;
        }
        else if(sampleFrequency>=userStopFreq-(frequencySpacing/2) && sampleFrequency<=userStopFreq+(frequencySpacing/2)) { //Fin du message
#if DEBUG
            NSLog(@"Stop");
#endif
        ...
        if (sampleFrequency>=userStartFreq-100 && sampleFrequency<userStartFreq+100 && !isInitiate) {   //Détection d'un début de message
#if DEBUG
            NSLog(@"Gogo");
#endif
...
-(void)startAnalysis
{
    ...
    [self analysisPhase1];
#if DEBUG
    NSLog(@"INIT P1 /**/**/**/ : %@",messageReceive);
#endif
    [self analysisPhase2];
#if DEBUG
    NSLog(@"INIT P2 /**/**/**/ : %@",messageReceive);
#endif
    [self analysisPhase3];
#if DEBUG
    NSLog(@"INIT P3 /**/**/**/ : %@",messageReceive);
#endif
...
-(BOOL)volumeControl
{
    if ([mySession outputVolume]<minimumVolume)
        return FALSE;
    else
        return TRUE;
}
...
#pragma mark - User Personalisation

-(void)personaliseFrequencyValue:(int)bandStartFrequency : (NSString*)yourAlpha{
    numberOfAlphaInDictionnary = (int)[yourAlpha length];
    userMinFreq = bandStartFrequency;
    userMaxFreq = 20000;
    
    [userDictionnary removeAllObjects];
    for (int i =0; i<numberOfAlphaInDictionnary; i++) {
        [userDictionnary addObject:[NSString stringWithFormat:@"%c",[yourAlpha characterAtIndex:i]]];
    }
    
    userStartFreq = userMinFreq - 200;
    userStopFreq = userMaxFreq + 200;
    
    userFreqEspacement = (userMaxFreq - userMinFreq)/numberOfAlphaInDictionnary;
}

-(int)getIndex:(NSString*)theChar{
    for (int i=0; i<[userDictionnary count]; i++) {
        if ([theChar isEqualToString:[userDictionnary objectAtIndex:i]]) {
            return i;
        }
    }
    return -1;
...
-(void)printASCIItoFrequencyTable{
    printf("=====Frequency to ASCII Array =====\n");
    for(int i =0;i<=[userDictionnary count];i++){
        @try {
            printf("%d  =>  %s    ",userMinFreq+userFreqEspacement*i,[[userDictionnary objectAtIndex:i] UTF8String]);
            i++;
            printf("%d  =>  %s\n",userMinFreq+userFreqEspacement*i,[[userDictionnary objectAtIndex:i] UTF8String]);
        }
        @catch (NSException *exception) {}

    }
    printf("Start : %d     Stop : %d\n",userStartFreq,userStopFreq);
    printf("===================================\n");
    /*
    int base = 18000;
    int diff = 18;
    printf("=====Frequency to ASCII Array =====\n");
    for(int i =0;i<=94;i++){
        printf("%d  =>  %c    ",base+i*diff,(char)((base + i*diff-17995)/18+32));
        i++;
        printf("%d  =>  %c\n",base+i*diff,(char)((base + i*diff-17995)/18+32));
    }
    printf("===================================\n");
    */
}

-(void)startEmitUniqueFrequency:(int)frequency{
    [self stopProcessingAudio];
    sendMonoFrequency=TRUE;
    receptionMode = FALSE;
...
-(BOOL)isSendingMonoFrequency{return sendMonoFrequency;}
