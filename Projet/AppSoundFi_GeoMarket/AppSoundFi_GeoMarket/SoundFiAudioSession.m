                                        ...
                                        SInt16                      *sampleBuffer)      // frame buffer
{
	...
	if (read > inNumberFrames) {
        ...
        for (int i=100; i<n; i+=2) {
            ...
            //printf("%d\n",(int)curFreq);
            if ((curFreq > dominantFrequency) && (curFreq >1000000000)) {
...
-(void)emissionSampleCalcul:(int)frequence : (int)numFrames : (Float32 *)buffer
{
    ...
    if (frequence == 19390 && amplitude < 0.8)
        amplitude+=0.01;
    else if (frequence == 20960 && amplitude > 0)
        amplitude-=0.01;
...
-(void)sampleTreatment:(int)numFrames
{
    ...
    else
    {
        ...
        if (sampleFrequency>=19900 && sampleFrequency<21000 && !isInitiate)
        {
            receptionQuality=0;
...
-(void) zoneDetection:(int*)data
{
    ...
    if ([lastPos isEqualToString:currentPos] && ![lastPosValidate isEqualToString:currentPos]) { //TEST
        ...
        //NSString* urlContents = @"Promotion en cours";
        [self.delegate localisationData:lieu :urlContents];
        
        #if DEBUG
            NSLog(@"%@",urlContents);
            currentPos=@"Nop"; // Pour test à supprimer
        #endif
    }
    lastPos=currentPos;
...
-(void)checkTimeOut
{
    if (isInitiate &&compteur>800) {    // Déclencher au bout de 13 secondes sans réception ancien 200
        NSLog(@"TimeOut,sortie de zone");
        isInitiate=FALSE;
        nbrVal=0;
        [self zoneDetection:nil];
    }
}
...
-(int)initAudioSession
{
    ...
    [mySession setPreferredIOBufferDuration: (1024.0/sampleRate) error: &audioSessionError];
...
-(BOOL)isRunning{return engineIsRunning;}
