...
OSStatus renderCallback(void *userData,
    ...
    if(THIS->receptionMode)
    {
        ...
        if(THIS->isInitiate || THIS->geoIsInitiate)
        {
            THIS->compteur=compteur+1;
        }
        
        if (THIS->paiementMode) {
            THIS->compteurProcess++;
...
-(void)changeGeolockTimer:(int)duree;                                               //Allow change the timer (fixed to 30 sec at launch)

-(void)endPaiementProcess;                                                          //Call at the end of a paiement processing

-(void)activateSoundFiLocalisation;                                                 //Activate the Géolocalisation function
-(void)activateSoundFiMessaging;                                                    //Activate the messaging function
-(void)activateSoundFiPaiement;                                                     //Activate the paiement function

-(void)desactivateSoundFiLocalisation;                                              //Desactivate the Géolocalisation function
-(void)desactivateSoundFiMessaging;                                                 //Desactivate the messaging function
-(void)desactivateSoundFiPaiement;                                                  //Desactivate the paiement function
@end
...
-(SoundFiAudioSession*)init
{
    ...
    geolocalisationMode=TRUE;
    simpleMessagingMode=TRUE;
    paiementMode=FALSE;
    enableBackground=TRUE;
    
    nbrEchantillon=2048; //2048
...
/** This method deal with the initilisation of the paiement variables
 
 @see init
 */
-(void)initPaiement{
    timeOutProcess=20000;
#if DEBUG
    myID=@"{id_user:\"134513455\"";
    cryptedPassAES=@"tRbVQ9KjTWqcOQ9r7pyCUI7hkMdg8f13lISsT0H6y9s=";
#endif
}
...
/** This method deal with the initilisation of the audioEngine's localisation
 
 Init geolocalisation variable like the LocationManager and configure the NSTimer for a periodic update of the user location. By Default, the
 user considered out of a soundFi covered spot.
...
-(void)initGeoloc{
    ...
    [zoneArray addObject:@"Chambre Florian"];
    [zoneArray addObject:@"Sallon"];
    [zoneArray addObject:@"Cuisine"];
#endif
    
    curentFrequencyData = malloc(sizeof(int)*25);
    for (int i=0; i<25; i++) {curentFrequencyData[i]=1;}
    
    [self zoneDetection:curentFrequencyData];
    ...
    //Timer qui seras déclenché toutes les X minutes pour récupérer une position précise
    timer = [NSTimer scheduledTimerWithTimeInterval:geolocktimer
                                             target:self
...
-(int)initAudioSession
{
    ...
    [mySession setPreferredIOBufferDuration: ((float)nbrEchantillon/sampleRate) error: &audioSessionError]; //Number of sample must depend of the mode WALE 2048
    // and so the fft size in the code 256 -> 2048 everywhere
...
-(void)sampleTreatment:(int)numFrames {
    ...
    if (geolocalisationMode)
        [self geolocalisationReceptionSampleTreatment];
    
    if (simpleMessagingMode)
        [self messagingReceptionSampleTreatment];
    
    if (paiementMode)
        [self paiementReceptionSampleTreatment];
}

/**---------------------------------------------------------------------------------------
 * @name Reception methods
 * GeolocalisationSampleTreatment
 *  ---------------------------------------------------------------------------------------
 */

/** Treatment of geolocalisation frequency.
 
 @see sampleTreatment
 */
-(void)geolocalisationReceptionSampleTreatment{
    if(geoIsInitiate) {
        if(sampleFrequency>20000 && sampleFrequency<21000){
            if (nbrVal>24) {
                nbrVal=0;
                [self zoneDetection:curentFrequencyData];
            }
            else {
                curentFrequencyData[nbrVal]=sampleFrequency;
                nbrVal++;
                compteur=0;
            }
        }
        else if (sampleFrequency>=17650 && sampleFrequency<17950 && simpleMessagingMode){       // Détection d'un message
            //Arret de la géolocalisation
            geolocalisationMode=FALSE;
            geoIsInitiate=FALSE;
            
            //Changmement de la rate
            nbrEchantillon=256;
            [mySession setPreferredIOBufferDuration: ((float)nbrEchantillon/sampleRate) error: nil];
            
            //Démarage de la réception
        ...
        if (sampleFrequency>=19900 && sampleFrequency<21000 && !geoIsInitiate) {
            compteur=0;
            geoIsInitiate=TRUE;
...
-(void)messagingReceptionSampleTreatment{
    ...
    if(isInitiate) {
        ...
        if (sampleFrequency>=17995 && sampleFrequency<=19710) {                 //Ajout d'un caractère à la chaine finale
            ...
            //Réactivation de la géoloc
            geolocalisationMode=TRUE;
            compteur=0;
            
            //Changement de la rate pour la géoloc
            nbrEchantillon=2048;
            [mySession setPreferredIOBufferDuration: ((float)nbrEchantillon/sampleRate) error: nil];
        ...
        if (sampleFrequency>=17650 && sampleFrequency<17950 && !isInitiate) {   //Détection d'un début de message
            ...
            //Coupure de la géoloc
            geolocalisationMode=FALSE;
            geoIsInitiate=FALSE;
            
            //Changement de la rate d'écoute
            nbrEchantillon=256;
            [mySession setPreferredIOBufferDuration: ((float)nbrEchantillon/sampleRate) error: nil];
...
/**---------------------------------------------------------------------------------------
 * PaiementReceptionSampleTreatment
 *  ---------------------------------------------------------------------------------------
 */
/** Treatment if paiement frequency (very similar to messaging frequency)
 
 @see sampleTreatment
 */
-(void)paiementReceptionSampleTreatment{
...
#pragma mark - Geolocalisation methods


/**---------------------------------------------------------------------------------------
 * @name Geolocalisation methods
 * ZoneDetection
 *  ---------------------------------------------------------------------------------------
 */
/** Use to deal with the location of the user in function of the received frequency.
 
 Made he average frequency of the last 100 frequency record, and then check the standart deviation to determine if the samples are concluding
 
 NOTE:May use GPS localisation in futur implementation.
 
 @param data The array of frequency (int* 100)
 @see geolocalisationReceptionSampleTreatment
 @exception outRange Can raise an NSExeption if the location is out range of the location's array
 @exception outZone Raise if the user is not in a zone under soundFi technologie
 */
-(void) zoneDetection:(int*)data
{
    
    //Vérifie si on est en sortie de zone
    if(data==nil){
        NSLog(@"Zone non couverte");
        [self.delegate localisationData:@"Hors Zone" :@"Nop"];
        return;
    }
    ...
    for (int i=0; i<25; i++) {avrgFreq+=data[i];}
    avrgFreq/=25;
    
    //Calcul de l'écart type pour détecter les passages entre 2 zones
    for (int i=0; i<25; i++) {ecartType+=pow(data[i]-avrgFreq,2);}
    ecartType=sqrt(ecartType/25.);
    
    //Vérification de la cohésion des diverse fréquences
    if(ecartType>25){
        NSLog(@"Echantillon non concluant / Passage entre 2 zones");
        printf("Frequence moyenne :%d     Ecart type:%f\n",avrgFreq,ecartType);
        return;
    }
    
    //Détermine sa position en fonction de la fréquence
    nbrSpotInZone=20;
    @try {
        lieu=[zoneArray objectAtIndex:(avrgFreq-20000)/(1000/nbrSpotInZone)];
    }
    @catch (NSException *outRange) {
        return;
    }
    currentPos=lieu;
    
    //Si position validé, on l'envoie au serveur qui renvoie une promotion
    if ([lastPos isEqualToString:currentPos] && ![lastPosValidate isEqualToString:currentPos]) { //TEST
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            url = [NSURL URLWithString:[NSString stringWithFormat:@"http://mynetshare.fr/test.php?id=%d",avrgFreq]];
            urlContents = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil];
            [self sendNotification:urlContents];
#if DEBUG
            NSLog(@"%@",urlContents);
            currentPos=@"Nop";
#endif
 ...
 *  ---------------------------------------------------------------------------------------
 */
/** Delegate method for LocationManager to update user location.
 
 Pop a notification in background processing and add the promotion in the promotion array.
 
 */
-(void)sendNotification:(NSString*)data{
    
    NSString *promotionText=@"Null";
    NSString *imgUrl=@"Null";
    
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"\"image_url\":.*\"," options:0 error:NULL];
    NSTextCheckingResult *match = [regex firstMatchInString:data options:0 range:NSMakeRange(0, [data length])];
    
    imgUrl=[data substringWithRange:[match rangeAtIndex:0]];
    imgUrl=[[imgUrl componentsSeparatedByString:@"\""]objectAtIndex:3];
    
    regex = [NSRegularExpression regularExpressionWithPattern:@"\"Promotion\":.*" options:0 error:NULL];
    match = [regex firstMatchInString:data options:0 range:NSMakeRange(0, [data length])];
    
    promotionText=[data substringWithRange:[match rangeAtIndex:0]];
    promotionText=[[promotionText componentsSeparatedByString:@"\""]objectAtIndex:3];
    
    
    [self.delegate localisationData:imgUrl :promotionText];
    
    //Background notification
    UILocalNotification* localNotification = [[UILocalNotification alloc] init];
    localNotification.fireDate = [NSDate dateWithTimeIntervalSinceNow:1];
    localNotification.alertBody = promotionText;
...
/** Delegate method for LocationManager to update user location.
 ...
 For more detail, take a look at [locationManaerUpdate](https://developer.apple.com/library/ios/documentation/CoreLocation/Reference/CLLocationManagerDelegate_Protocol/CLLocationManagerDelegate/CLLocationManagerDelegate.html#//apple_ref/occ/intfm/CLLocationManagerDelegate/locationManager:didUpdateLocations:)
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
    ...
    if (isHightAccuracy) {
        ...
        if ([urlContents isEqualToString:@"inZone"]) {
            isInZone=TRUE;
            if (![self engineIsRunning] && [self backgroundIsEnable])
                [self startAudioUnit:SFReceivingMode :nil];                                     //On démare l'arrière plan
        }
        else{
            UIApplicationState state = [[UIApplication sharedApplication] applicationState];
            isInZone=FALSE;
            if ([self engineIsRunning] && state==UIApplicationStateBackground)
                [self stopProcessingAudio];                                                     //On stop l'arrière plan
        }
        });
...
/**---------------------------------------------------------------------------------------
 * ChangeLocationManagerAccuracy
 *  ---------------------------------------------------------------------------------------
 */
/** Increse the accuracy of the GPS for 1 location.
 
 Call by the timer all the n seconds to get a precise location of the user.
 ...
 *  ---------------------------------------------------------------------------------------
 */
/** Permit to change the interval of the timer for user localisation.
 
 Currently can be called by everyone but may become a private method
 
 @param duree The new value representing the time between to fire of the timer
 
 */
-(void)changeGeolockTimer:(int)duree{
    geolocktimer=duree;
    [timer invalidate];
    timer = [NSTimer scheduledTimerWithTimeInterval:geolocktimer
                                             target:self
...
-(void)checkTimeOut
{
    ...
    if (geoIsInitiate)
        ...
        limite=300;
    else if(paiementMode)
        limite=700;
    
    
    if ((geoIsInitiate || isInitiate) && compteur>limite) {    //The time out must depend of the mode use WALE
#if DEBUG
        if (geoIsInitiate)
            NSLog(@"TimeOut geolocating");
        else if (isInitiate)
        ...
        //Réactivation du mode géo et changement de la rate
        geolocalisationMode=TRUE;
        nbrEchantillon=2048;
        [mySession setPreferredIOBufferDuration: ((float)nbrEchantillon/sampleRate) error: nil];
    ...
    if (paiementMode && compteur>limite){
#if DEBUG
        NSLog(@"TimeOut");
#endif
        [self endPaiementProcess];
...
{
    if (compteurProcess>timeOutProcess) {
        NSLog(@"TimeOut Processing");
        compteurProcess=0;
        [self.delegate messageReceived:@"TimeOut"];
        [self transactionUpdate:@"TimeOut"];
    }
...
-(void)startAnalysis
{
    ...
    if (paiementMode)
        [self transactionUpdate:[NSString stringWithString:messageReceive]];
    messageReceive=[[NSMutableString alloc]init];
}
 ...
 * ActivateSoundFiLocalisation
 *  ---------------------------------------------------------------------------------------
 */
/** Activate the location mode
 
 @see desactivateSoundFiLocalisation
 @see localisationModeIsEnable
 */
-(void)activateSoundFiLocalisation{
    geolocalisationMode=TRUE;
    [locationManager startUpdatingLocation];}
...
/**---------------------------------------------------------------------------------------
 * ActivateSoundFiPaiement
 *  ---------------------------------------------------------------------------------------
 */
/** Activate the paiement mode
 
 @see desactivateSoundFiPaiement
 @see paiementModeIsEnable
 */
-(void)activateSoundFiPaiement{paiementMode=TRUE;}

/**---------------------------------------------------------------------------------------
 * DesactivateSoundFiLocalisation
 *  ---------------------------------------------------------------------------------------
 */
/** Desactivate the location mode
 
 @see activateSoundFiLocalisation
 @see locationModeIsEnable
 */
-(void)desactivateSoundFiLocalisation{
    geolocalisationMode=FALSE;
...
/**---------------------------------------------------------------------------------------
 * DesactivateSoundFiPaiement
 *  ---------------------------------------------------------------------------------------
 */
/** Desactivate the paiement mode
 
 @see activateSoundFiPaiement
 @see paiementModeIsEnable
 */
-(void)desactivateSoundFiPaiement{paiementMode=FALSE;}
...
-(BOOL)localisationModeIsEnable{return geolocalisationMode;}
...
/**---------------------------------------------------------------------------------------
 * PaiementModeIsEnable
 *  ---------------------------------------------------------------------------------------
 */
/** Check if the paiement mode is enable
 
 @return TRUE if the paiement mode is enable
 @see activateSoundFiPaiement
 @see desactivateSoundFiPaiement
 */
-(BOOL)paiementModeIsEnable{return paiementMode;}
...
/** Call to perform background task
 ...
 This function is called when the aplication is entering background. It does geolocalisation processing to determine if we have or not to stop the audioEngine in background
 
 @see enteredForeground
 */
-(void)enteredBackground{
#if DEBUG
    NSLog(@"I go background tasking");
#endif
    simpleMessagingMode=FALSE;
...
/** Call to perform background task
 ...
 This function is called when the aplication is entering foreground. It does geolocalisation processing to determine if we have or not to relaunch the audioSession when starting foreground processing.
 
 @see enteredBackground
 */
-(void)enteredForeground{
#if DEBUG
    NSLog(@"Incoming for foreground task");
#endif
    simpleMessagingMode=TRUE;
...
/** Call to perform background task
 ...
 Control state of the transaction and send message according to the state. This is the main paiement engine's method.
 
 @param theMessage The last message received by the cash register.
 @warning Although this method work, they could be some bug. Moreover, send a classic message during processing could pertubate the transaction.
 */
- (void) transactionUpdate:(NSString*) theMessage{
    
    compteurProcess=0;
    usleep(40000);
    if ([theMessage isEqualToString:@"TimeOut"]) {
        timeOut=TRUE;
    }
    else
        timeOut=FALSE;
    switch (transactionState) {
        case 0: {                                                       // Acquitement du prix reçu
            paiementInformation=theMessage;
            timeOutProcess=516;  //Time out de 3 sec set
            transactionState++;
            dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self startAudioUnit:SFSendingMode:theMessage];
            });
        }
            break;
        case 1:                                                        // Si on reçoit "GP" le prix reçu est bon et on acquite GP sinon retour 0
            if (timeOut) {
                transactionState--;
                [self transactionUpdate:paiementInformation];
            }
            else {
                if ([theMessage isEqualToString:@"GoodPrice"]) {
                    [self.delegate transactionData:paiementInformation];// Delegate pour envoyer le prix à l'UI
                    timeOutProcess=20000;
                    ...
                    transactionState--;
                    [self transactionUpdate:theMessage];
                }
            }
            
            break;
        case 2: {                                                         // Envoie de mon id à la caisse
            timeOutProcess=516;   // Set du timeOut à 3 sec soit 516 frames
            dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self startAudioUnit:SFSendingMode:myID];
            });
            transactionState++;
        }
            break;
        case 3:                                                         // Vérifie que idAcquit==idEnvoyé, si ok "GID" sinon on renvoie
            if (timeOut) {
                transactionState--;
                [self transactionUpdate:nil];
            }
            else {
                if ([theMessage isEqualToString:myID]) {
                    //NSLog(@"Il a reçu le bon ID");
                    timeOutProcess=20000;   //Remet le timeOut à l'infini (si déclenché gros problème)
                    transactionState++;
                    //[sw_id setOn:TRUE];
                    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [self startAudioUnit:SFSendingMode:@"GoodId"];
                    });
                }
                else {
                    transactionState--;
            ...
            break;
        case 4: {                                                         //Acquitement du statut reçu
            transactionState++;
            paiementStatut=theMessage;
            [self startAudioUnit:SFSendingMode:paiementStatut];
        }
            break;
        case 5:                                                         //Si messageReçu=="GF" fin de la transaction sinon renvoi du statut
            if ([theMessage isEqualToString:@"GoodEnd"]) {
#if DEBUG
                NSLog(@"Transaction réalisée avec succès");
#endif
                [self.delegate transactionFinalState:paiementStatut];
                transactionState++;
                timeOutProcess=20000;
    ...
    [self.delegate transactionStatutChange:transactionState];
    
}

//Use to ask user's password
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    @try {
        myID=[AESCrypt encrypt:[NSString stringWithFormat:@"%@,pass:\"%@\"}",myID,[[alertView textFieldAtIndex:0] text]]
                      password:[AESCrypt decrypt:cryptedPassAES password:[[alertView textFieldAtIndex:0]text]]]; // Point sensible
    }
    @catch (NSException * e) {
        transactionState=-15;
        return;
...
-(void)startpaiementProcessing{
    ...
    //Démarrage du mode de paiement
    paiementMode=TRUE;
}


/**---------------------------------------------------------------------------------------
 * endPaiementProcessing
 *  ---------------------------------------------------------------------------------------
 */
/** Call at end of paiement
 
 This method permit to stop processing a paiement, it disable the paiement mode and enable the other mode. Do stuff like reset the timer for geolock etc . . .
 @see startpaiementProcessing
 */
-(void)endPaiementProcess{
#if debug
    NSLog(@"Fin du paiement");
#endif
    //Désactivaton du paiement
    paiementMode=FALSE;
    isInitiate=FALSE;
    
    //Réactivation des autres services
    simpleMessagingMode=TRUE;
    geolocalisationMode=TRUE;
    timer = [NSTimer scheduledTimerWithTimeInterval:geolocktimer
                                             target:self
