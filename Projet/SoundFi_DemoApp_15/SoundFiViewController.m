...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        if (promo==nil)
            promo = [[NSMutableArray alloc]init];
    }
    return self;
}
...
- (void) localisationData:(NSString*)imgPromo :(NSString*)txtPromo{
    NSString *theMessage = txtPromo;
    promoIncomming = theMessage;
    promoImgIncomming = imgPromo;
    [promo addObject:theMessage];
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.4 animations:^{ self.dragView.alpha = 0; } completion:^(BOOL finished) {
            [self addDragView:@"Promotion"];
            [UIView animateWithDuration:0.4 animations:^{
                self.dragView.alpha = 1.;
            } completion:nil];
        }];    });
...
- (void)addDragView:(NSString*) type
{
    ...
    if ([type isEqualToString:@"Message"]) {
        ...
        NSURL *imageURL = [NSURL URLWithString:promoImgIncomming];
        NSData *imageData = [NSData dataWithContentsOfURL:imageURL];
        UIImage *imagePromo = [UIImage imageWithData:imageData];
        UIImageView *imagePromoView = [[UIImageView alloc] initWithImage:imagePromo];
        imagePromoView.frame = CGRectMake(292.5-imagePromoView.frame.size.width,0, imagePromoView.frame.size.width, imagePromoView.frame.size.height );
        ...
        [self.dragView addSubview:noNotifIcone];
        UILabel *noNotifLabel = [[UILabel alloc] initWithFrame:CGRectMake(13 + 36.5, 0, 292.5 - 49.5, 40)];
        noNotifLabel.text = @"Nouvelle promotion";
        noNotifLabel.centerY = noNotifIcone.centerY - 10;
        noNotifLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:20];
        noNotifLabel.textColor = [UIColor colorWithRed:(234./255.) green:(115./255.) blue:(17./255.) alpha:1.];
        [self.dragView addSubview:noNotifLabel];
        ...
        content.editable = NO;
        content.userInteractionEnabled = NO;
        content.text = promoIncomming;
        content.backgroundColor = [UIColor clearColor];
        [self.dragView addSubview:content];
        [self.dragView addSubview:imagePromoView];
        self.dragView.alpha = 0.;
