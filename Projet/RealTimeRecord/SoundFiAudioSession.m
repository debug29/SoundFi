...
-(SoundFiAudioSession*)init
{
    ...
    //tabTstFreq=malloc(sizeof(int)*200000);
    tabIndex=0;
...
-(void)sampleTreatment:(int)numFrames
{
    if(isInitiate)
    {
        ...
        if (sampleFrequency>20487 && sampleFrequency<20512)
        {
            geoLock++;
            [self.delegate startingReception];
        }
    ...
    {
        fftGetFrequencyHighAccuracy( (__bridge void*)self, numFrames, samplesBuffer);    //Background
        if (sampleFrequency>=19300 && sampleFrequency<21000 && !isInitiate)
        {
            NSLog(@"Gogo");
            [self.delegate startingReception];
            compteur=0;
