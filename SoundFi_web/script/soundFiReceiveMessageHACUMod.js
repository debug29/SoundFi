  /**
    ...
    * de fréquence spécifique de 18kHz à 20kHz avec 18 Hz d'interval.
  ...
  */
  ...
  var node =null;
  var buffer = [];
  var mypitchshift = null;

  var DEBUG = false;

  var messageInReception = "";

  var callbackPeriod = 80;
  ...
  function errorStream(){
    ...
    /**
      * @function
      * @name update
      * @description Cette fonction est appelé régulièrement et permet la récupération périodique des informations en provenance du graph audio, elle procède en plus
        à une analyse des données temporelles de l'échantillon audio par pitch shifting.
      * @see {@link raf}
      * @see {@link Pitchshift}
    */
  function update(){
    ...
    var hightaccuFreq = mypitchshift.process((0.5 * 1.5) + .5, 2048, 8, data);

    if(hightaccuFreq>17500){
      if (DEBUG)
        console.log(hightaccuFreq);
      if (hightaccuFreq>17700 && hightaccuFreq<17900 && !messageStart) {
        messageInReception = "";
        messageStart = true;
        callbackPeriod=40;
      ...
      else if(hightaccuFreq>20100 && hightaccuFreq<20300 && messageStart){
        messageStart = false;
        console.log("Stop");
        console.log(messageInReception);
        callbackPeriod=40;
        analysis(messageInReception);
      }
      else{
        dateTimeOut=new Date();
        //console.log(String.fromCharCode(32 + Math.round(((hightaccuFreq-18000)/18))) + "  " + hightaccuFreq);
        if (DEBUG)
          console.log(hightaccuFreq);
        if (hightaccuFreq>17900 && hightaccuFreq<18100){
          messageInReception= messageInReception + "0";
        }
        else if (hightaccuFreq>18100 && hightaccuFreq<18300){
          messageInReception= messageInReception + "1";
        }
        else if (hightaccuFreq>18300 && hightaccuFreq<18500){
          messageInReception= messageInReception + "2";
        }
        else if (hightaccuFreq>18500 && hightaccuFreq<18700){
          messageInReception= messageInReception + "3";
        }
        else if (hightaccuFreq>18700 && hightaccuFreq<18900){
          messageInReception= messageInReception + "4";
        }
        else if (hightaccuFreq>18900 && hightaccuFreq<19100){
          messageInReception= messageInReception + "5";
        }
        else if (hightaccuFreq>19100 && hightaccuFreq<19300){
          messageInReception= messageInReception + "6";
        }
        else if (hightaccuFreq>19300 && hightaccuFreq<10500){
          messageInReception= messageInReception + "7";
        }
        else if (hightaccuFreq>19500 && hightaccuFreq<19700){
          messageInReception= messageInReception + "8";
        }
        else if (hightaccuFreq>19700 && hightaccuFreq<19900){
          messageInReception= messageInReception + "9";
        }
        else {
          messageInReception= messageInReception + "";
        }
      }
  ...
  function raf(callback) {
    setTimeout(callback, callbackPeriod);
  ...
  function kill(){}
    ...
    /**
      ...
      * @param {string} message_ Le message non traité qui à été reçu lors de la phase d'écoute et qui va être parsé afin de supprimé les erreurs
        et les doublons.
      * @see {@link update}
    */
  function analysis(message_){
    ...
    if (DEBUG == 2){
      while(i<message_.length-3){
        var firstChar = message_[i];
        var secondChar = message_[i+1];
        var thirdChar = message_[i+2];

        if ((firstChar == thirdChar) && (secondChar != firstChar)) {
          message_[i+1] = message_[i];
          console.log('plop ' + i + "  " + message_[i] + "  to  " + message_[i+1]);
          i = i + 2;
        }
        else if((firstChar == secondChar) && (thirdChar != secondChar)){
          if(Math.abs(thirdChar - secondChar) <2){
            message_[i+2]=message_[i+1];
            i = i + 2;
          }
        }
        else if((thirdChar == secondChar) && (firstChar != secondChar)){
          if(Math.abs(firstChar - secondChar) <2){
    ...
    while(i<message_.length-2){
      currentChar = message_.charAt(i);
      nextChar = message_.charAt(i+1);
      if (currentChar == nextChar) {
        finalMessage = finalMessage + currentChar;
        lastValideChar = currentChar;
        i = i + 2;
        while(message_.charAt(i) == lastValideChar){
          i++;
          repeatCount++;
          if (repeatCount>8) {
            finalMessage = finalMessage + currentChar;
  ...
  function info(){
    var data = new Float32Array(2048);
    analyser.getFloatTimeDomainData(data);
    mypitchshift.process(mypitchshift, 2048, 4, data);
  }
  ...
  navigator.getUserMedia( {audio:true}, gotStream,errorStream);
    ...
    /**
      * @function
      * @name pitchshift
      * @description Cette fonction procède à l'analyse d'un échantillon audio par FFT, en appliquant en plus une fenêtre et un pitch shifting afin de maximiser la
        précision de la fréquence résultante.
      * @param {Number} fftFrameSize La taille de l'échantillon (multiple de 2)
      * @param {Number} sampleRate Le ratio audio (44100Hz) 
      * @param {String} algo Le type d'algo utilisé ici, mettre FFT
      * @see {@link getReady}
    */
function Pitchshift(fftFrameSize, sampleRate, algo) {
  if( arguments.length ) { this.getready(fftFrameSize, sampleRate, algo); }
    ...
    /**
      * @function
      * @name pitchshift.getready
      * @description Cette fonction prépare le terrain afin de réaliser des FFT sur des échantillon
      * @param {Number} fftFrameSize La taille de l'échantillon (multiple de 2)
      * @param {Number} sampleRate Le ratio audio (44100Hz)
      * @param {String} algo Le type d'algo utilisé ici, mettre FFT 
      * @see {@link getReady}
...
Pitchshift.prototype.getready = function (fftFrameSize, sampleRate, algo) {
    ...
    /**
      * @function
      * @name pitchshift.process
      * @description Cette fonction lance une FFT sur un échantillon audio
      * @param {pitchShift} Une instance de pitchShift préalablement init
      * @param {Number} numSampToProcess Le nombre de valeur dans un échantillon
      * @param {Number} osamp OverSample value, je conseil d'utilisé 8, cela augmente la précision au maximum mais utilise plus de ressource.
      * @param {Float32*} inData Un pointeur vers le tableau de l'échantillon
      * @return {Number} frequency La fréquence dominante déduite de l'échantillon
      * @see {@link getReady}
    */
Pitchshift.prototype.process = function (pitchShift, numSampsToProcess, osamp, indata) {
    ...
    for (j = 0; j < numSampsToProcess; j++){
        ...
        if (this.gRover_ >= this.fftFrameSize_) {
            ...
            for (k = 0 ; k < this.fftFrameSize_ ; k++) {
                ...
                /*        
                for (var i = 0; i < this.gAnaMagn.length; i++) {
                          console.log(this.gAnaMagn[i]);
                };        
                for (var i = 0; i < this.gAnaFreq.length; i++) {
                          console.log(this.gAnaFreq[i]);
                };    
                */
                var maxMag = 1.2;
                var displayFreq = 0.;
                ...
                for (var k = 0; k < fftFrameSize2; k++) {
                    if ((this.gAnaMagn[k] > maxMag)&& (k>100)) {                   //Analysing only the hight frequency REP
                    maxMag = this.gAnaMagn[k];
                    displayFreq = this.gAnaFreq[k];
                    }
                };
                //console.log(maxMag);
                this.freqTotal+=displayFreq;
                this.pitchCount++;
    ...
    return(Math.round(this.freqTotal/this.pitchCount));
}
...
//                                                                                            //
//                                                                                            //
//  Tous ce qui se trouve après viens de la lib dsp.js, importé salement en copier collé afin //
//  de pouvoir utilisé les ressources, par soucis de rapiditée. Il faudrait créer une         //
//  dépendance.                                                                               //
//                                                                                            //
//  Attention : une ligne de code importante se trouve à la fin du fichier et permet l'init   //
//  du pitchShifting.                                                                         //
//                                                                                            //
//                  mypitchshift = new Pitchshift(2048,44100,"FFT");                          //                                 
//                                                                                            //
//                                                                                            //
////////////////////////////////////////////////////////////////////////////////////////////////
