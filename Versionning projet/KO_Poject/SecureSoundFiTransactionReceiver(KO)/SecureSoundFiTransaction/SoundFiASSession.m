...
OSStatus RecordCallback(void *userData,
    ...
    if (!THIS->transactionIsInit) {
        ...
        else if (THIS->sampleFrequency>=18000 && THIS->sampleFrequency<=19692) {
    ...
    if (THIS->startingMessageSize) {
        if (THIS->nbrAquitInit==0 && THIS->sampleFrequency >18000 && THIS->sampleFrequency<19699 && !THIS->isAGoogLetter) {
            THIS->previousCarac=THIS->currentCaracFrequency;
            THIS->currentCaracFrequency=18000 + (((THIS->sampleFrequency-17991)/18+32)-32)*18;
            NSLog(@"            %c         %d",(char)((THIS->currentCaracFrequency-17995)/18+32),THIS->sampleFrequency);
            //printf("Sending : %d   %d \n",THIS->sampleFrequency,((THIS->sampleFrequency-17991)/18+32));
        }

        if(THIS->sampleFrequency>19846 && THIS->sampleFrequency <19854 && !THIS->isAGoogLetter) {
            [THIS->messageReceive appendFormat:@"%c",(char)((THIS->previousCarac-17995)/18+32)];
            NSLog(@"DEALWITHIT :   %@",THIS->messageReceive);
            THIS->isAGoogLetter=TRUE;
        }
        else if (THIS->isAGoogLetter) {      //Est ce que une nouvelle lettre est envoyé?  Attent sur la freq d'init
            if(THIS->sampleFrequency>=19707 && THIS->sampleFrequency<=19713) {
                //NSLog(@"On reppart");
