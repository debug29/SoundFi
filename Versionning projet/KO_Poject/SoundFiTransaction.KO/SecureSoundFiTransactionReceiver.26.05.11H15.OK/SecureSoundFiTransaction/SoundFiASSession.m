...
OSStatus RecordCallback(void *userData,
    ...
    if (!THIS->transactionIsInit) {
        if (THIS->sampleFrequency>=19702 && THIS->sampleFrequency<=19718) {
            //Fréquence de démarage reçu
            THIS->frequenceInitReceive=TRUE;
            THIS->nbrAquitInit=0;
            NSLog(@"RECEPTEUR : Init");
        }
    ...
    if (THIS->startingMessageSize) {
        if (THIS->nbrAquitInit==0 && THIS->sampleFrequency >18000 && THIS->sampleFrequency<19699) {
        ...
        if(THIS->sampleFrequency>19741 && THIS->sampleFrequency <19759 && !THIS->isAGoogLetter) {
