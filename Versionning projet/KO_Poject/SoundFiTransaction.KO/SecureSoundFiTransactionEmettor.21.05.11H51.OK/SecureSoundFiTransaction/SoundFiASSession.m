...
OSStatus RecordCallback(void *userData,
    ...
    if (!THIS->transactionIsInit) {
        if (THIS->sampleFrequency>=19405 && THIS->sampleFrequency<19415) {
            //Fréquence de démarage reçu
            NSLog(@"EMETTEUR : Acquittement reçu");
            THIS->transactionIsInit=TRUE;
            THIS->startingMessageSize=TRUE;
        }
    }
    
    //Receiving number acquittal
    if (THIS->sampleFrequency>=17980 && THIS->sampleFrequency<=18155 && THIS->startingMessageSize) {
        //NSLog(@"EMETTEUR : Je compare la frequence reçu avec la fréquence envoyé");
        int freqTemp=(THIS->sampleFrequency+300)-(18000 +(((int)[THIS->messageSize characterAtIndex:THIS->currentDigitIndex]-32)*18));
        //NSLog(@"Freq reçu : %d   //   Freq envoyé : %d   //  Dif : %d",THIS->sampleFrequency,18000 +((int)[THIS->messageSize characterAtIndex:THIS->currentDigitIndex]-32)*18,freqTemp);
        if (freqTemp>-3 && freqTemp<3) {
            NSLog(@"Acquitement chiffre validé");
            if (THIS->currentDigitIndex < [THIS->messageSize length]) { // Est ce que on a envoyé le nombre entier ?
                THIS->currentDigitIndex++;
            }
            else {                                                      // Si oui, on a fini la phase d'envoie de la taille
                THIS->startingMessageSize=FALSE;
            }
        }
    }
...
OSStatus PlayBackCallback(void *userData,
    ...
    //Sending size of the message digit by digit
    if (THIS->startingMessageSize) {
        //NSLog(@"EMETTEUR : J'envoie un digit");
        if (THIS->nbrRepeat<2 && THIS->currentDigitIndex<[THIS->messageSize length]) {
            frequence=18000 +(((int)[THIS->messageSize characterAtIndex:THIS->currentDigitIndex]-32)*18);
            //NSLog(@"Frequency : %d",frequence);
            THIS->nbrRepeat++;
        }
    }
