...
OSStatus RecordCallback(void *userData,
    ...
    if (!THIS->transactionIsInit) {
        if (THIS->sampleFrequency>=19402 && THIS->sampleFrequency<19418) {
            //Fréquence de démarage reçu
            NSLog(@"EMETTEUR : Acquittement init reçu");
    ...
    if(THIS->startingMessageSize && !THIS->acquitementCaracIsOk) {
        ...
        if (freqTemp>-9 && freqTemp<9) {                            // L'acquittement est validé
    ...
    if(THIS->acquitementCaracIsOk && THIS->sampleFrequency+300>19725) { // Si l'acquitement de la lettre est validé et que la
        int freqTemp=THIS->sentFrequency-THIS->sampleFrequency-300;     // fréquence reçu est proche de celle de validation.
        //NSLog(@"MaFreq : %d / LaFreqRecu : %d  / LaTemp : %d",THIS->sentFrequency,THIS->sampleFrequency,freqTemp);
        if (freqTemp>-9 && freqTemp<9) {                                // L'acquittement est validé
