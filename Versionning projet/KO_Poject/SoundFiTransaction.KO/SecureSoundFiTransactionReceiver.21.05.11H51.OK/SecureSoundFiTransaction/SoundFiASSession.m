...
OSStatus RecordCallback(void *userData,
    ...
    if (!THIS->transactionIsInit) {
        if (THIS->sampleFrequency>=19707 && THIS->sampleFrequency<=19713) {
            ...
            if (THIS->frequenceInitReceive) {
                ...
                THIS->nbrAquitInit=0;
                NSLog(@"RECEPTEUR : Début de la réception de la taille du message");
            }
        }
    }
    ...
    if (THIS->startingMessageSize) {
        //NSLog(@"RECEPTEUR : Réception d'un chiffre de la taille du message");
        if (THIS->sampleFrequency>=18280 && THIS->sampleFrequency<=18455) {
            THIS->currentCarac=(char)((THIS->sampleFrequency-17991)/18+32);
            //NSLog(@"Frequency : %d , carac : %c",THIS->sampleFrequency,THIS->currentCarac);
        }
    }
...
OSStatus PlayBackCallback(void *userData,
    ...
    if (THIS->frequenceInitReceive && THIS->nbrAquitInit<2 && !THIS->transactionIsInit) {
        frequence=19410;
        THIS->nbrAquitInit++;
    }
    ...
    if (THIS->startingMessageSize && THIS->nbrAquitInit<2) {
        //NSLog(@"J'acquite le chiffre reçu");
        frequence=18000 + (((int)THIS->currentCarac -32)*18) - 300;
        THIS->nbrAquitInit++;
    }
