...
//  EMMETOR FREQUENCY = RECEIVEUR FREQUENCY -300
/////////////////
OSStatus RecordCallback(void *userData,
    ...
    //On écoute pas si on envoie
    //if (THIS->nbrRepeat<2) {
    //    THIS->sampleFrequency=0;
    //}
    
    //Receiving initialisation acquittal
    if (!THIS->transactionIsInit) {
        if (THIS->sampleFrequency>=19407 && THIS->sampleFrequency<19414) {
            //Fréquence de démarage reçu
            //NSLog(@"EMETTEUR : Acquittement init reçu pour %c",(char)[THIS->theMessage characterAtIndex:THIS->currentDigitIndex]);
            THIS->transactionIsInit=TRUE;
            THIS->startingMessageSize=TRUE;
        }
    }
    
    //Acquitement du chiffre envoyé
    if(THIS->startingMessageSize && !THIS->acquitementCaracIsOk) {
        int freqTemp=THIS->sentFrequency-THIS->sampleFrequency-300;
        //NSLog(@"MaFreq : %d / LaFreqRecu : %d  / LaTemp : %d",THIS->sentFrequency,THIS->sampleFrequency,freqTemp);
        if (freqTemp>-3 && freqTemp<3) {                            // L'acquittement est validé
            THIS->acquitementCaracIsOk=TRUE;
            //NSLog(@"EMETTEUR : Caractère validé");
        }
    }
    
    //Acquitement de la réussite de l'échange
    if(THIS->acquitementCaracIsOk && THIS->sampleFrequency+300>19701) { // Si l'acquitement de la lettre est validé et que la
        int freqTemp=THIS->sampleFrequency-19550;                       // fréquence reçu est proche de celle de validation.
       // NSLog(@"MaFreq : %d / LaFreqRecu : %d  / LaTemp : %d",THIS->sentFrequency,THIS->sampleFrequency,freqTemp);
        if (freqTemp>-3 && freqTemp<3) {                                // L'acquittement est validé
            THIS->acquitementsendingIsOk=TRUE;
            //NSLog(@"EMETTEUR : Message validé");
...
OSStatus PlayBackCallback(void *userData,
    ...
    //Sending initialisation's frequency
    if (!THIS->transactionIsInit) {
        /*
        if (THIS->nbrRepeat<2) {
            frequence = 19710;
            THIS->sentFrequency=frequence;
            THIS->nbrRepeat++;
        }
         */
        frequence = 19710;
    ...
    if (THIS->startingMessageSize) {
         ...
         */
        frequence = 18000 +(((int)[THIS->theMessage characterAtIndex:THIS->currentDigitIndex]-32)*18);;
        THIS->sentFrequency = frequence;
    }
    
    //Envoie de la freq de réussite
    if (THIS->acquitementCaracIsOk) {
        /*
        if (THIS->nbrRepeat<2) {
            frequence = 19750;
            THIS->sentFrequency = frequence;
...
-(void)rebootSending
{
    ...
    acquitementCaracIsOk=FALSE;
    acquitementsendingIsOk=FALSE;
    
    //On incrémentera ici l'index du message
}
...
-(SoundFiASSession*)init
{
    ...
    theMessage=@"Bonjour_Francois_je_test";
...
-(void)emissionSampleCalcul:(int)frequence : (int)numFrames : (Float32 *)buffer
{
    ...
    if (compteur<3 && acquitementCaracIsOk) {
        amplitude=0.7;
    }
