...
-(void)startAnalysis
{
    ...
    NSLog(@"Mon message /**/**/**/**/**/**/ %@",messageReceive);
...
-(int)startAudioUnit:(int)mode
{
    ...
    int i=3;
    int doublette=0;
    tab_de_freq=malloc(sizeof(int)*([myMessage length]*2 + 6));
    tab_de_freq[0]=19390;
    tab_de_freq[1]=19390;
    tab_de_freq[2]=19390;
    ...
    while(i<[myMessage length]*2-3)
    {
        printf("%d         %d\n",i,compteur);
        tab_de_freq[i]=20000 +(((int)[myMessage characterAtIndex:compteur]-32)*10);
        doublette++;
        if (doublette==2) {
            compteur++;
            doublette=0;
    ...
    tab_de_freq[i]=20960;i++;
    tab_de_freq[i]=20960;i++;
    tab_de_freq[i]=20960;
    
    for (int k=0; k<([myMessage length]*2 + 6); k++) {
        printf("Freq : %d carac: %c \n",tab_de_freq[k],[myMessage characterAtIndex:compteur]);
    }
    */
