...
//			NSLog(@"after transform");
            ...
            /*
            if(THIS->tabIndex<200000){
                for (int i = 0; i < fftFrameSize;i++)
                {
                   THIS->tabTstFreq[THIS->tabIndex]=gFFTworksp[i];
                    THIS->tabIndex++;
                }
		...
		//	smbFft(gFFTworksp, fftFrameSize, -1);
            ...
            for (k = 0; k <= fftFrameSize2/6; k++) {
			 ...
			 displayFreq = 0.0;
			 for (k = 0; k <= fftFrameSize2; k++) {
			 if ((gAnaMagn[k] > maxMag)) {           //Analysing only the hight frequency REP   && (k>fftFrameSize2/6)
			 maxMag = gAnaMagn[k];
			 displayFreq = gAnaFreq[k];
				}
			 }
