...
-(void) zoneDetection:(int*)data
{
    ...
    if(data==nil){
        NSLog(@"Sortie de Zone");
        NSURL* url = [NSURL URLWithString:[NSString stringWithFormat:@"http://mynetshare.fr/test.php?id=%d",-100]];
        NSString* urlContents = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil];
        //NSString* urlContents = @"Pas de promotion";
        [self.delegate localisationData:@"Hors Zone" :urlContents];
    ...
    for (int i=0; i<50; i++) {
        ecartType+=pow(data[i]-avrgFreq,2);
    }
    ...
    //Cherche localement à valider sa position

    if(avrgFreq>20000 && avrgFreq<20050){
        lieu=[NSString stringWithFormat:@"Chambre Florian"];
    }
    else if (avrgFreq>20050 && avrgFreq<20100){
        lieu=[NSString stringWithFormat:@"Sallon"];
    }
    else if (avrgFreq>20100 && avrgFreq<20150){
        lieu=[NSString stringWithFormat:@"Cuisine"];
    }
    currentPos=lieu;
