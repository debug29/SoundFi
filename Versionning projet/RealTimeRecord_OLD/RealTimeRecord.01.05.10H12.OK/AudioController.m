...
//Emission get frequency method
-(int)getASCIIFrequency
{
    int frequence;
    
    if (compteur < [myMessage length]) {
        frequence=19500 +(((int)[myMessage characterAtIndex:compteur]-32)*10);
...
-(void)sampleTreatment:(int)numFrames
{
    if(isInitiate)
    {
        ...
        if (sampleFrequency>=19495 && sampleFrequency<=20455)
        {
            NSString *res = [NSString stringWithFormat:@"%c",(char)((sampleFrequency-19495)/10+32)];
            //NSLog(@" Freq : %d  ASCII: %3d Carac : %@",sampleFrequency,(sampleFrequency-18490)/20+32,res);
            [messageReceive appendString:res];
            compteur=0;
        }
        else if(sampleFrequency>=20455 && sampleFrequency<=20465)
        {
            NSLog(@"Stop");
            [self startAnalysis];
    ...
    {
        fftGetFrequencyLowAccuracy( (__bridge void*)self, numFrames, samplesBuffer);
        if (sampleFrequency>=19500 && sampleFrequency<=20400)
        {
            NSLog(@"Gogo");
            compteur=0;
            isInitiate=TRUE;
...
{
    /*
    outputBuffer = (float*)malloc(256 *sizeof(float));
	analysisBuffer = (float*)malloc(256 *sizeof(float));
...
-(void)analysisPhase5
{
    ...
    while (i<[messageReceive length]-3)
    {
        ...
        if ([un isEqualToString:deux] && [un isEqualToString:trois]) {
...
-(void)startAnalysis
{
    /*
    messageReceive=[[NSMutableString alloc]init];
    [messageReceive appendString:@" r|{{{ysttttvzyyyyvpppppkeeeee/     ):::::,     &12111)     @iiiiijmmmmmhaaaaacgggggfeeeeec_____guuuuuutttttpiiiii0     +:::::,     !\"\"\"\"\"AhhhhhltttttttttttspppppU:::::5///////////KsssssroooooquuuuundddddefffffgiiiiiJ.....Fcccccgooooonmmmmm=/////Fiiiiijmmmmmhaaaaacgggggfeeeeejsssss>/////3666666777776555554333332111114566667888887666662.....Fjjjjjlppppplggggg0\"\"\"\"\"&,,,,,'     /tttttneeeeekxxxxxwttttt2     +:::::,     !\"\"\"\"\"1CBBBBUoooooonnnnnljjjjjloooooquuuuutrrrrr=,,,,,'     >bbbbbeiiiiiheeeeehonnnnqvvvvvneeeeehnnnnnquuuuuneeeee/     /ssssstuuuuutrrrrr2     8SSSSSoooooquuuuurnnnnnjdddddXGFFFFSiiiiiJ.....'     0DCCCCQeeeeedccccceiiiii0     ?eeeeejssssssttttt2     6uuuuurnnnnnkeeeee/     .mmmmmjeeeeeissssssssssslaaaaadgggggfeeeee/     ?ddddddeeeee.     /tttttneeeeejssssssttttt2     /ppppppoooooquuuuutrrrrr2     0uuuuusnnnnnjeeeee.     0tttttlaaaaadhiiiijlllllllllllieeeee.     0sssssstttttlaaaaafnnnnnjdddddcaaaaagrrrrrsttttt2     /eeeeehnnnnn1     /nnnnnnooooonmmmmmibbbbbgrrrrrleeeee/     ?ddddddeeeee.     ?cccccbaaaaagrrrrrkaaaaabcccccitttttneeeeejrrrrrleeeeejsssss>.....'     5LLLLL9'''''Fooooojbbbbbejjjjjheeeeedcccccitttttpiiiiihff  ueeeee/     ?  eeeeeissssssssssslaaaaacgggggfeeeee/     .pppppjaassssssmeeeee/     /ssssskaaaaafnnnnnpsssss2     ?eeeeejrrrrrrrrrrrleeeeekuuuuutrrrrrrsssss2     ?eeeeejttttt2     >aaaaaivvvvvneeeeedccccc.     /tttttroooooquuuuutsssss2     Blllllieeeeejsssss2     ?cccccbaaaaagrrrrrjaaaaabcccccitttttmeeeeejrrrrrleeeeejsssss>.....(\"\"\"\"\"9}}}}}~"];
    */
...
-(void)startSampleProcess{
    ...
    [NSTimer scheduledTimerWithTimeInterval:0 target:self.delegate
